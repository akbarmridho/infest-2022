...
const TimelineItem = ({orientation, name, date} : Props) => {
    return (<>
        <div className="hidden sm:flex items-center relative text-gray-100">
            { orientation === 'left' && 
                <>
                <div className="h-[5px] bg-gradient-to-r from-tyellow-300 to-tyellow-100 w-full"></div>
                <h4 className="font-tommy text-2xl md:text-3xl absolute bottom-11 right-2 text-right">{name}</h4>
        ...
        <div className={`items-center relative text-gray-100 ${orientation === 'left' ? 'flex sm:hidden' : 'flex'}`}>
                ...
                <h4 className="font-tommy text-2xl md:text-3xl absolute bottom-11 left-2 text-left">{name}</h4>
