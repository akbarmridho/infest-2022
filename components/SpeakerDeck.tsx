...
interface Props {
    speakers : {
        [key: string] : SpeakerProps[]
    }
}

const SpeakerDeck = ({ speakers }: Props) => {
    const [activeItems, setActiveItems] = useState<string>(Object.keys(speakers)[0])
    return (
                ...
                <div className="mt-6">
                    {
                        speakers[activeItems].map((speaker, i) => {
                            return <Speaker key={`${i}-${activeItems}`} name={speaker.name} topic={speaker.topic} imageUrl={speaker.imageUrl} pos={ i % 2 === 0 ? 'left' : 'right'}/>
                        })
                    }
                </div> 
            </div>
